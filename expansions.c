...
/*   Updated: 2025/12/24 01:07:49 by mohamaib         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	initialize(t_val_full_init *val_init, t_segment *seg, t_gc *gc)
{
	...
	if (seg->str[val_init->i] == '$')
		val_init->i++;
	handle_special_expan(val_init, seg, gc);
	val_init->expand_val[val_init->j] = '\0';
	find_next_dollar(val_init, seg);
}

void	initialize2(t_val_full_init *val_init, t_segment *seg, t_gc *gc,
		t_env **env)
{
	val_init->j = 0;
	val_init->full = gc_ft_strdup("", gc);
	val_init->trailing_val = gc_malloc(sizeof(char) * ((val_init->z
					- val_init->i) + 1), gc);
	...
	val_init->full = ft_strjoin_plus(val_init->leading_val,
			replace_val_in_env(val_init->expand_val, env, gc),
			val_init->trailing_val, gc);
	val_init->full_all = gc_ft_strjoin(val_init->full_all, val_init->full, gc);
}
