...
void					handle_quoted_word(char *str, t_token **head, int *i,
							t_gc *gc);
void					handle_regular_word(char *str, t_token **head, int *i,
							t_gc *gc);
void					process_redir_target(t_token *token, char *str, int *i,
							t_gc *gc);

/* parser.c */
t_node					*create_cmd_node(char **cmd, t_gc *gc);
t_node					*create_redir_node(t_token_type type, t_token *red_tok,
							t_node *left, t_gc *gc);
...
void					init_buffers(t_val_full_init *val_init, t_segment *seg,
							t_gc *gc);
...
void					write_expanded(t_segment seg, int fd, t_env **env,
							t_gc *gc);
...
void					process_heredoc_content(char *line,
							t_heredoc_vars *here);
void					heredoc_child(const char *delim, int expand, t_gc *gc,
							int p[2]);
